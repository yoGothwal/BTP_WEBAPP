...
class Application extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            Transfer_Functions: [
                {
                    id: "Reference site: bedrock to surface",
                    color: "hsl(120, 100%, 20%)",
                    data: [
                        { x: 1, y: 2 },
                        { x: 2, y: 5 },
                ...
                {
                    id: "Target site: bedrock to surface",
                    color: "hsl(16, 88%, 54%)",
                    data: [
                        { x: 1, y: 2 },
                        { x: 2, y: 5 },
                ...
                {
                    id: "Reference site's surface to target site's surface",
                    color: "hsl(147, 50%, 47%)",
                    data: [
                        { x: 1, y: 2 },
                        { x: 2, y: 5 },
            ...
            FAS: [
                {
                    ...
                    data: [
                        ...
                        { x: 6.044, y: 0.02419 },
                        { x: 7.043, y: 0.022034 },
                        { x: 8.042, y: 0.01976333 },
                        { x: 9.041, y: 0.01773167 },
                        { x: 10.04, y: 0.01586 },
                        { x: 11.039, y: 0.014222 },
                        { x: 12.038, y: 0.012762 },
                        { x: 13.037, y: 0.011363 },
                        ...
                        { x: 16.034, y: 0.008316 },
                        { x: 17.033, y: 0.007467 },
                        { x: 18.032, y: 0.0066756 },
                        { x: 19.031, y: 0.0059976 },
                        { x: 20.03, y: 0.005396 },
                        { x: 21.029, y: 0.0047984 },
                        { x: 22.028, y: 0.0042688 },
                        { x: 23.027, y: 0.00381055 },
                        { x: 24.026, y: 0.0034118 },
                        { x: 25.025, y: 0.0030225 },
                        { x: 26.024, y: 0.0027056 },
                        { x: 27.023, y: 0.00240425 },
                        { x: 28.022, y: 0.0021556 },
                        { x: 29.021, y: 0.0019158 },
                        { x: 30.02, y: 0.001706 },
                        { x: 31.019, y: 0.00153715 },
                        { x: 32.018, y: 0.0013664 },
                        { x: 33.017, y: 0.00122245 },
                        { x: 34.016, y: 0.0010884 },
                        { x: 35.015, y: 0.00097905 },
                        { x: 36.014, y: 0.00087088 },
                        { x: 37.013, y: 0.00077557 },
                        { x: 38.012, y: 0.00069553 },
                        { x: 39.011, y: 0.00061897 },
                        { x: 40.01, y: 0.00055443 },
                        { x: 41.009, y: 0.00049555 },
                        { x: 42.008, y: 0.00044196 },
                        { x: 43.007, y: 0.00039703 },
                        { x: 44.006, y: 0.00035576 },
                        { x: 45.005, y: 0.00031815 },
                        { x: 46.004, y: 0.0002832 },
                        { x: 47.003, y: 0.00025418 },
                        { x: 48.002, y: 0.00022595 },
                        { x: 49.001, y: 0.00020298 },
                        { x: 50, y: 0.000181 },
                    ],
                },
            ], // FAS File Contents
    ...
    handleChange = (event) => {
        ...
        if (inputName === "Target_Depth") {
            ...
            inputName === "MaxFreq" ||
            inputName === "WavFrac" ||
            inputName === "Ref_Halfspace_Vs" ||
            inputName === "Ref_Halfspace_Damping" ||
            inputName === "Ref_Water_Table_Depth" ||
            inputName === "Tar_Halfspace_Vs" ||
            inputName === "Tar_Halfspace_Damping" ||
            inputName === "Tar_Water_Table_Depth"
        ) {
            this.setState({ [inputName]: parseFloat(inputValue) });
        } else {
        ...
        if (inputName === "Motion_File") {
            ...
            if (inputValue === "Sample_Motion")
                Motion_Data[1].data = Sample_Motion.data;
            if (inputValue === "Kobe") Motion_Data[1].data = Kobe.data;
            if (inputValue === "Northridge") Motion_Data[1].data = Northridge.data;
            if (inputValue === "LomaGilroy") Motion_Data[1].data = LomaGilroy.data;
            if (inputValue === "Parkfield") Motion_Data[1].data = Parkfield.data;
            this.Generate_Motion();
        }
    ...
    readExcelProfileData = (event) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                // Determine the worksheet name based on inputName
                ...
                // Process the data
                const Site_Soil_Profile_Data_Array = data
                    .slice(1)
                    .map((row, index) => ({
                        Name: index + 1,
                        Thickness: parseFloat(row[1]),
                        Vs: parseFloat(row[2]),
                        Gamma: parseFloat(row[3]),
                        PI: parseFloat(row[4]),
                        OCR: parseFloat(row[5]),
                        Damping: parseFloat(row[6]),
                        SoilModel: parseInt(row[7]),
                    }));

                // Update state based on inputName
                if (inputName === "ReferenceDataFile") {
                    this.setState(
                        {
                            Reference_Site_Soil_Profile: Site_Soil_Profile_Data_Array,
                        },
                        () => {
                            this.update_Target_Site_Profile_Plots();
                            console.log(
                                "Updated soilProfile in state:",
                                this.state.Reference_Site_Soil_Profile);
                            this.update_Reference_Site_Profile_Plots()
                        }
                    );
                } else if (inputName === "TargetDataFile") {
                    this.setState(
                        {
                            Target_Site_Soil_Profile: Site_Soil_Profile_Data_Array,
                        },
                        () => {
                            console.log("Updated soilProfile:", this.Target_Site_Soil_Profile);
                            this.update_Target_Site_Profile_Plots();
                        }
                    );
                }
    ...
    downloadInputMotionFile = (event) => {
        ...
        const Reference_Transfer_Functions =
            this.state.Transfer_Functions[0]["data"];
        const Reference_to_Target_Transfer_Functions =
            this.state.Transfer_Functions[2]["data"];
        n = Target_Transfer_Functions.length;

        for (let i = 0; i < n; i++) {
            freq3[i] = [Target_Transfer_Functions[i].x];
            Transfer_Functions[i] = [
                Reference_Transfer_Functions[i].y,
                Target_Transfer_Functions[i].y,
                Reference_to_Target_Transfer_Functions[i].y,
            ];
        }
        ...
        const Reference_Max_Strain_Profile =
            this.state.Max_Strain_Profile[0]["data"];

        n = Target_Max_Strain_Profile.length;
        for (let i = 0; i < n; i++) {
            Target_Max_Strain_Profile_Array[i] = [
                Target_Max_Strain_Profile[i].y,
                Target_Max_Strain_Profile[i].x,
            ];
        }

        n = Reference_Max_Strain_Profile.length;
        for (let i = 0; i < n; i++) {
            Reference_Max_Strain_Profile_Array[i] = [
                Reference_Max_Strain_Profile[i].y,
                Reference_Max_Strain_Profile[i].x,
            ];
        }
        ...
        XLSX.utils.sheet_add_aoa(
            ws,
            [["Time (s)", "Reference site", "Target site"]],
            { origin: "A2" }
        );
        ...
        XLSX.utils.sheet_add_aoa(ws, [["Frequency smplitude spectrum (g-s)"]], {
            origin: "E1",
        });
        XLSX.utils.sheet_add_aoa(
            ws,
            [["Frequency (Hz)", "Reference site", "Target site"]],
            { origin: "E2" }
        );
        ...
        XLSX.utils.sheet_add_aoa(ws, [["Pseudo spectral acceleration (g)"]], {
            origin: "I1",
        });
        XLSX.utils.sheet_add_aoa(
            ws,
        ...
        XLSX.utils.sheet_add_aoa(ws, [["Transfer functions (g-s)"]], {
            origin: "A1",
        });
        XLSX.utils.sheet_add_aoa(
            ws,
            [
                [
                    "Frequency (Hz)",
                    "Reference site: bedrock to surface",
                    "Target site: bedrock to surface",
                    "Reference site's surface to target site's surface",
                ],
            ],
            { origin: "A2" }
        );
        ...
        XLSX.utils.sheet_add_aoa(ws, [["Maximum shear strain profile (%)"]], {
            origin: "F1",
        });
        XLSX.utils.sheet_add_aoa(
            ws,
            [["Depth (m)", "Reference site", "Depth (m)", "Target site"]],
            { origin: "F2" }
        );
        XLSX.utils.sheet_add_aoa(ws, Reference_Max_Strain_Profile_Array, {
            origin: "F3",
        });
        XLSX.utils.sheet_add_aoa(ws, Target_Max_Strain_Profile_Array, {
            origin: "H3",
        });
    ...
    update_Target_Depth_Plots = (Target_Depth) => {
        ...
        const Vs_Profile_Data = Site_Vs_Profile_Data[0].data.concat(
            Site_Vs_Profile_Data[1].data
        );
        var maxValue = Math.max.apply(
            null,
            Vs_Profile_Data.map(function (o) {
                return o.x;
            })
        );
        ...
        var Target_Depth_Vs_Data = [
            { x: 0, y: Target_Depth },
            { x: maxValue * 1.1, y: Target_Depth },
        ];

        const Damping_Profile_Data = Site_Damping_Profile_Data[0].data.concat(
            Site_Damping_Profile_Data[1].data
        );
        maxValue = Math.max.apply(
            null,
            Damping_Profile_Data.map(function (o) {
                return o.x;
            })
        );
    ...
    update_Reference_Site_Soil_Profile = (newData) => {
        this.setState({ Reference_Site_Soil_Profile: newData }, () => {
            // This ensures the state is updated before running the graph updates
            this.update_Reference_Site_Profile_Plots();
            this.update_Target_Depth_Plots(this.state.Target_Depth);
        });
    };
    ...
    render() {
        const {
            step,
            Ref_Halfspace_Vs,
            Ref_Halfspace_Damping,
            Ref_Water_Table_Depth,
            Tar_Halfspace_Vs,
            Tar_Halfspace_Damping,
            Tar_Water_Table_Depth,
            Analysis_Type,
            Tol,
            MaxIter,
            EffStrain,
